...
findOpponent = function () {
  var board = findBoard();
  if (board && board.PlayerA === Session.get("id")) return UserSessions.findOne({"id": board.PlayerB});
  if (board && board.PlayerB === Session.get("id")) return UserSessions.findOne({"id": board.PlayerA});
};

Template.board.helpers({
  board: function () {
    if(Boards.findOne({"PlayerA": Session.get("id")}) !== undefined) return Boards.findOne({"PlayerA": Session.get("id")});
    else if(Boards.findOne({"PlayerB": Session.get("id")}) !== undefined) return Boards.findOne({"PlayerB": Session.get("id")});
  },
  you: function () {
  ...
  turn: function () {
    ...
    if (board) {
      ...
      if (board.turn === "AWin" && UserSessions.findOne({"id": board.PlayerA}).id === Session.get("id"))
        return "You have won the game.";
      if (board.turn === "AWin" && UserSessions.findOne({"id": board.PlayerA}).id !== Session.get("id"))
        return UserSessions.findOne({"id": board.PlayerA}).name + " has won the game.";
      if (board.turn === "BWin" && UserSessions.findOne({"id": board.PlayerB}).id === Session.get("id"))
        return "You have won the game.";
      if (board.turn === "BWin" && UserSessions.findOne({"id": board.PlayerB}).id !== Session.get("id"))
